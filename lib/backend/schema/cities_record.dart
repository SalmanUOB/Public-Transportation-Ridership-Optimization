...
class CitiesRecord extends FirestoreRecord {
  ...
  // "governID" field.
  DocumentReference? _governID;
  DocumentReference? get governID => _governID;
  bool hasGovernID() => _governID != null;

  void _initializeFields() {
    _name = snapshotData['name'] as String?;
    _governID = snapshotData['governID'] as DocumentReference?;
  }
...
class CitiesRecordDocumentEquality implements Equality<CitiesRecord> {
  ...
  bool equals(CitiesRecord? e1, CitiesRecord? e2) {
    return e1?.name == e2?.name && e1?.governID == e2?.governID;
  }

  @override
  int hash(CitiesRecord? e) =>
      const ListEquality().hash([e?.name, e?.governID]);
